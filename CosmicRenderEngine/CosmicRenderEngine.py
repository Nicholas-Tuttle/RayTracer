...
class CosmicRenderEngine(bpy.types.RenderEngine):
    ...
    def render(self, depsgraph):
        ...
        print("Resolution: X: " + str(resolution_x) + ", Y: " + str(resolution_y))
        print("Resolution scale " + str(scale))
        ...
        for object_instance in depsgraph.object_instances:
            ...
            if object_instance.object is not None:
                mesh = object_instance.object.to_mesh()
                if mesh is None:
                    # Happens for non-geometry objects.
                    continue
                print(f"Exporting mesh with {len(mesh.vertices)} vertices at {object_instance.matrix_world}")
        ...
        if self.is_preview:
            ...
            for i in range(0, pixel_count):
                rect[i] = [0.0, 0.0, 1.0, 1.0]
                rect[i][0] = horizontal / self.size_x
                rect[i][1] = vertical / self.size_y
                horizontal += 1.0
                horizontal %= self.size_x
